...
<template>
    ...
    <ul v-else class="questions">
        <li class="questions__question" v-for="question in questions" :key="question.uuid">
            <router-link :to="{ name: 'question', params: { uuid: question.uuid } }" class="question-card">
                <h3 class="question-card--title">{{ question.title }}</h3>
                <div class="question-card__tags">
                    <router-link :to="{ name: 'tag', params: { slug: tag.slug } }" v-for="tag in question.tags"
                        :key="tag.slug" class="tag-badge">{{ tag.label }}</router-link>
                </div>
